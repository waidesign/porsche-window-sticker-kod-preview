...
export default function FeatureGrid() {
  ...
  return (
    <section id="features" className="py-24 bg-zinc-950 border-b border-zinc-900 no-print">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {features.map((feat) => (
            <div
              key={feat.id}
              className="bg-zinc-900/40 p-6 border border-zinc-800 hover:border-zinc-700 transition-all duration-300 relative group rounded-none"
            >
              <div className="w-10 h-10 bg-zinc-950 border border-zinc-800 flex items-center justify-center mb-5 group-hover:border-[#A1FF2C] transition-colors duration-300">
                {feat.icon}
              </div>
              <h3 className="font-display text-base font-bold text-white mb-2 group-hover:text-zinc-200 transition-colors">
                {feat.title}
              </h3>
              <p className="text-xs text-zinc-400 leading-relaxed">
                {feat.desc}
              </p>
              <div className="absolute top-0 right-0 w-0 h-[1.5px] bg-[#A1FF2C] transition-all duration-300 group-hover:w-full"></div>
            </div>
          ))}
        </div>
      </div>
